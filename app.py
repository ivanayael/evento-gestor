...
def upload_file():
    ...
    if not file.filename.endswith(('.xls', '.xlsx')):
        return jsonify({'error': 'El archivo debe ser un archivo de Excel (.xls, .xlsx)'}), 400
...
def export_file():
    try:
        ...
        # Falta pasar la tabla
        df['Link de Google Site'] = 'URL del HTML'  # O algún cálculo
        df['Link de Invitacion'] = 'LINK DEL HTML'  # O algún cálculo
